 ...
 * Copyright (c) 2022 RoScripter999 and contributors
 *
...
// Only import pure modules that don't import other parts of Velocity.
...
const setCssDebounced = debounce((css: string) => VelocityNative.quickCss.set(css));

const themeStore = DataStore.createStore("VelocityThemes", "VelocityThemeData");

// probably should make this less cursed at some point
window.VelocityNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/RoScripter999/Velocity" }),
    ...
    quickCss: {
        get: () => DataStore.get("VelocityQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("VelocityQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "VelocityQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => VelocityNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("VelocitySettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("VelocitySettings", JSON.stringify(s)),
