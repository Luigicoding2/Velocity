...
export interface ThemeDef {
    name: string;
    themeActivationModes: ThemeActivationMode;
    enabled?: boolean;
}
...
export interface Settings {
    ...
    themes: {
        onlineThemes: ThemeDef[];
        onlineThemesEnabled: boolean;
        localThemes: ThemeDef[];
    };
    ...
    rendererPerformanceBoost: boolean;
    plugins: {
        [plugin: string]: {
            enabled: boolean;
            [setting: string]: any;
...
const DefaultSettings: Settings = {
    ...
    rendererPerformanceBoost: false,
    plugins: {},
...
if (!IS_REPORTER) {
    SettingsStore.addGlobalChangeListener((_, path) => {
        ...
        VelocityNative.settings.set(SettingsStore.plain, path);
    });
}
...
export function migratePluginName(oldName: string, newName: string) {
    const { plugins } = SettingsStore.plain;
    if (newName in plugins) return;
