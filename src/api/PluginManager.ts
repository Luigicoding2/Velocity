...
import { type DefinedSettings, OptionType, type Patch, type Plugin, type PluginDef, type PluginSettingDef, ReporterTestable, StartAt } from "@utils/types";
...
export function hasAnyVisibleSettings({ settings }: Plugin) {
    return !!settings && Object.values(settings.def).some(s => !isSettingHidden(settings, s) && s.type !== OptionType.CUSTOM);
}

export function addPatch(newPatch: Omit<Patch, "plugin">, pluginName: string, pluginPath = `Velocity.Plugins.plugins[${JSON.stringify(pluginName)}]`) {
...
export const startPlugin = traceFunction("startPlugin", function startPlugin(p: Plugin) {
    const {
        ...
        chatBarButton, renderMemberListDecorator, renderMessageAccessory, renderMessageDecoration, messagePopoverButton, headerBarButton
    } = p;
    ...
    if (contextMenus) {
        ...
        for (const navId in contextMenus) {
            const def = contextMenus[navId];

            if (typeof def === "function") {
                addContextMenuPatch(navId, def, p.name, false);
            } else {
                addContextMenuPatch(navId, def.render, p.name, def.required ?? false);
            }
        }
    }
    ...
    if (userProfileBadge) {
        if (Array.isArray(userProfileBadge))
            userProfileBadge.forEach(b => addProfileBadge(b));
        else
            addProfileBadge(userProfileBadge);
    }
    ...
    if (chatBarButton) addChatBarButton(name, chatBarButton.render, chatBarButton.icon, chatBarButton.required ?? false);
...
export const stopPlugin = traceFunction("stopPlugin", function stopPlugin(p: Plugin) {
    ...
    if (commands?.length) {
        ...
        for (const cmd of commands) {
            try {
                if (cmd.options?.[0]?.type === ApplicationCommandOptionType.SUB_COMMAND) {
                    cmd.options.forEach(o => {
                        unregisterCommand(`${cmd.name} ${o.name}`);
                    });
                } else {
    ...
    if (contextMenus) {
        ...
        for (const navId in contextMenus) {
            ...
            const render = typeof def === "function" ? def : def.render;
            removeContextMenuPatch(navId, render);
        }
    }
    ...
    if (userProfileBadge) {
        if (Array.isArray(userProfileBadge))
            userProfileBadge.forEach(b => removeProfileBadge(b));
        else
            removeProfileBadge(userProfileBadge);
    }
