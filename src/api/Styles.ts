...
export const styleMap = window.VelocityStyles ??= new Map();

export const velocityRootNode = document.createElement("velocity-root");
/**
 * Houses all Velocity core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("velocity-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("velocity-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("velocity-user-styles");
...
export function initStyles() {
    ...
    const rendererCssNode = createAndAppendStyle("velocity-css-core", coreStyleRootNode);
    createAndAppendStyle("velocity-margins", coreStyleRootNode).textContent = generateMarginCss();

    VelocityNative.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        VelocityNative.native.onRendererCssUpdate(newCss => {
            rendererCssNode.textContent = newCss;
        });
    }

    VelocityNative.themes.getSystemValues().then(values => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(velocityRootNode);
}, { once: true });
