...
export type ContextMenuDef =
    ...
    | {
        render: NavContextMenuPatchCallback;
        required?: boolean;
    };
...
export const navPatches = new Map<string, Set<{ patch: NavContextMenuPatchCallback; pluginName?: string; required?: boolean; }>>();
export const globalPatches = new Set<GlobalContextMenuPatchCallback>();

/**
 * Automatically populated when plugins register context menu patches via addContextMenuPatch.
 * Used by UIElements to show/hide context menu patches per plugin.
 * Don't use this directly.
 */
export const ContextMenuButtonMap = new Map<string, ContextMenuButtonData>();

/**
 * Add a context menu patch
 * @param navId The navId(s) for the context menu(s) to patch
 * @param patch The patch to be applied
 * @param pluginName Optional plugin name, used to register the plugin in the UIElements context menu section
 */
export function addContextMenuPatch(navId: string | Array<string>, patch: NavContextMenuPatchCallback, pluginName?: string, required: boolean = false) {
...
export function removeContextMenuPatch<T extends string | Array<string>>(navId: T, patch: NavContextMenuPatchCallback): T extends string ? boolean : Array<boolean> {
    ...
    const results = navIds.map(id => {
        const patches = navPatches.get(id);
