    ...
    app.whenReady().then(() => {
        protocol.handle("velocity", ({ url: unsafeUrl }) => {
            ...
            switch (url) {
                ...
                case "velocityDesktopRenderer.js.map":
                case "preload.js.map":
                case "velocityDesktopPreload.js.map":
                case "patcher.js.map":
                case "velocityDesktopMain.js.map":
        ...
        try {
            if (RendererSettings.store.enableReactDevtools)
                installExt("fmkadmapgofadopljbjfkapdkoienihi")
                    .then(() => console.info("[Velocity] Installed React Developer Tools"))
                    .catch(err => console.error("[Velocity] Failed to install React Developer Tools", err));
        } catch { }
