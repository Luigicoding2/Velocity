...
export const shouldShowContributorBadge = (id: string, type?: "core" | "api") => {
    if (!isPluginDev(id) || DevsById[id].badge === false) return false;
    if (type === "core") return Object.values(Plugins).some(p => !p.name.endsWith("API") && p.required && p.authors?.some(a => String(a.id) === id));
    if (type === "api") return Object.values(Plugins).some(p => p.name.endsWith("API") && p.authors?.some(a => String(a.id) === id));
    return true;
};
