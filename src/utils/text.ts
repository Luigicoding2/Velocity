 ...
 * @param style The style of the duration

 */
export function formatDuration(time: number, unit: Units, short: boolean = false, style: style = "regular") {
    ...
    while (unitsAmounts.length) {
        ...
        if (amount === 0 && !res.length) continue;

        if (res.length) {
            res += style === "regular"
                ? (unitsAmounts.length ? ", " : " and ")
                : " ";
        }

        res += style === "regular"
            ? `${amount + getUnitStr(unit, amount === 1, short)}`
            : `${amount}${unit[0]}`;
    }

    return res || (
        style === "regular"
            ? `0${getUnitStr(unit, false, short)}`
            : "0" + unit[0]
    );
}
