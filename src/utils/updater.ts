...
export async function update() {
    ...
    const res = await Unwrap(VelocityNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(VelocityNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(VelocityNative.updater.getRepo());
