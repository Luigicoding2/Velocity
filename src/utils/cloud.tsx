...
export async function checkCloudUrlCsp() {
    ...
    if (host === "api.velocity.dev") return true;

    if (await VelocityNative.csp.isDomainAllowed(Settings.cloud.url, ["connect-src"])) {
        return true;
    }
...
export async function getAuthorization() {
    const secrets = await DataStore.get<Record<string, string>>("Velocity_cloudSecret") ?? {};

    const origin = getCloudUrlOrigin();

    // we need to migrate from the old format here
    if (secrets[origin]) {
        await DataStore.update<Record<string, string>>("Velocity_cloudSecret", secrets => {
            secrets ??= {};
