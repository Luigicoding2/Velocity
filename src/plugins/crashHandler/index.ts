...
import { closeAllModals, DraftType, ExpressionPickerStore, FluxDispatcher, hasAnyModalOpen, NavigationRouter, SelectedChannelStore } from "@webpack/common";
...
export default definePlugin({
    ...
    handleCrash(_this: any, errorState: any) {
        del("KeepCurrentChannel_previousData");
        if (IS_DEV) {
            try {
        ...
        setTimeout(() => {
                ...
                if (!shouldAttemptRecover) {
                    try {
                        showNotification({
                            ...
                            body: "Discord has crashed two times rapidly, not attempting to recover.",
                            noPersist: true
                        });
                    } catch { }
            ...
            try {
                if (!hasCrashedOnce) {
                    hasCrashedOnce = true;
                    maybePromptToUpdate("Uh oh, Discord has just crashed... but good news, there is a update available that might fix this issue! Would you like to update now?", true);
                }
            } catch { }
