...
import { DateUtils, RelationshipStore, Text, Tooltip } from "@webpack/common";
...
export default definePlugin({
    ...
    WrapperDateComponent: ErrorBoundary.wrap(({ user, children }: PropsWithChildren<{ user: User; }>) => {
        ...
        return <div className={cl("wrapper")}>
            {children}
            {!isNaN(since.getTime()) && (
                <Tooltip text={DateUtils.dateFormat(since, "LLLL")} tooltipClassName={cl("tooltip")}>
                    {props => (
                        <Text variant="text-xs/normal" className={cl("date")} {...props}>{formatter.format(since)}</Text>
                    )}
                </Tooltip>
            )}
        </div>;
