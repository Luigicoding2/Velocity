...
import { Avatar, ChannelActions, ChannelStore, Clickable, IconUtils, RelationshipStore, ScrollerThin, Text, useMemo, UserStore } from "@webpack/common";
...
function renderClickableGDMs(mutualDms: Channel[], onClose: () => void) {
    return mutualDms.map(c => (
        <Clickable
            ...
            onClick={() => {
                onClose();
                ChannelActions.selectPrivateChannel(c.id);
            }}
        >
            <Avatar
                    ...
                    : (
                        <div className={ProfileListClasses.empty}>
                            <div className={ProfileListClasses.textContainer}>
                                <Text color="text-strong" tag="h3" variant="text-md/medium" >You don't have any group chats in common</Text>
                            </div>
                        </div>
                    )
                }
