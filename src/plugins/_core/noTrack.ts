...
export default definePlugin({
    ...
    patches: [
        ...
        {
            find: "LIBDISCORE_RESET",
            replacement: [
                {
                    // Forces the internal 'i' variable (modded status) to false
                    match: /let \i=\(0,\i\.\i\)\(\);/g,
                    replace: "let $1=false;"
                },
                {
                    // Hardcodes the analytics property to false
                    match: /has_client_mods:\i/g,
                    replace: "has_client_mods:false"
                }
            ]
        },
