...
import { IS_WINDOWS, VELOCITY_GUILD_INVITE } from "@utils/constants";
...
async function syncSettings() {
    if (localStorage.Velocity_cloudSyncDirection === undefined) {
        // by default, sync bi-directionally
        localStorage.Velocity_cloudSyncDirection = "both";
    }
    // pre-check for local shared settings
    if (
        Settings.cloud.authenticated &&
        !await dsGet("Velocity_cloudSecret") // this has been enabled due to local settings share or some other bug
    ) {
        ...
        showNotification({
            ...
            onClick: () => SettingsRouter.openUserSettings("velocity_cloud_panel")
        });
        return;
    }
    ...
    if (
        ...
        localStorage.Velocity_cloudSyncDirection !== "manual" // if we're not in manual mode
    ) {
        if (localStorage.Velocity_settingsDirty && shouldCloudSync("push")) {
    ...
    const saveSettingsOnFrequentAction = debounce(async () => {
        if (Settings.cloud.settingsSync && Settings.cloud.authenticated) {
            await putCloudSettings();
            delete localStorage.Velocity_settingsDirty;
        }
    }, 60_000);

    SettingsStore.addGlobalChangeListener(() => {
        localStorage.Velocity_settingsDirty = true;
...
async function runUpdateCheck() {
    ...
    try {
        ...
        if (IS_DISCORD_DESKTOP) VelocityNative.tray.setUpdateState(isOutdated);
        ...
        if (Settings.autoUpdate) {
            ...
            if (Settings.autoUpdateNotification) {
                notify({
                    title: "Velocity has been updated!",
...
function initTrayIpc() {
    ...
    VelocityNative.tray.onCheckUpdates(async () => {
        try {
            const isOutdated = await checkForUpdates();
            VelocityNative.tray.setUpdateState(isOutdated);

            if (isOutdated) {
                showNotice("An Velocity update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VelocityNative.tray.onSupportServer(async () => {
        openInviteModal(VELOCITY_GUILD_INVITE);
    });

    VelocityNative.tray.onRepair(async () => {
        try {
            await update();
            relaunch();
        } catch (err) {
            UpdateLogger.error("Failed to repair Velocity", err);
        }
    });

    VelocityNative.tray.setUpdateState(getIsOutdated);
}
...
async function init() {
    ...
    if (IS_DEV) {
        const pendingPatches = patches.filter(p => !p.all && !p.lazy && p.predicate?.() !== false);
        if (pendingPatches.length)
            PMLogger.warn(
                "Webpack has finished initialising, but some patches haven't been applied yet.",
                "This might be expected since some Modules are lazy loaded, but please verify",
                "that all plugins are working as intended.",
                "You are seeing this warning because this is a Development build of Velocity.",
