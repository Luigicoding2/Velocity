...
This package provides TypeScript types for the Webpack modules of Discord's app.

## Installation

```bash
npm install -D @velocity-types
yarn add -D @velocity-types
pnpm add -D @velocity-types
```

## Example Usage

```ts
import type { UserStore } from "@velocity-types";
...
```ts
import { ApplicationCommandType } from "@velocity-types/enums";
...
The library also exports some RestAPI endpoints that you can use from Typescript code:
```ts
import { RestAPI } from "@webpack/common";

RestAPI.patch({
    url: "/users/@me/profile", // Autocomplete for some url endpoints
    body: {
        bio: "My bio"
    }
});
...
* Type definitions adapted from [Vencord](https://github.com/Vendicated/Vencord)
