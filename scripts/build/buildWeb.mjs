...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Velocity.ts"],
    format: "iife",
    globalName: "Velocity",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///VelocityWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/Velocity.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define Velocity prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'Velocity',{get:()=>Velocity});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Velocity.js": await readFile("dist/extension.js"),
        "dist/Velocity.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/Velocity.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Velocity.user.js", cssRuntime);
});
